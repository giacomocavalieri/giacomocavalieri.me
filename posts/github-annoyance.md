---
id = "how-i-solved-a-little-nuisance-using-gleam-and-a-raspberry-pi"
title = "How I solved a little nuisance using Gleam and a Raspberry Pi"
...
showing me the mergeability of any of my open PRs: that is wether they are still
drafts, if they have conflicts I need to address, or if they don't need any of
my attention.

![screenshot of GitHub's page showing one's open PRs](/imgs/github-prs-view.png)

Whenever a PR is merged, I need to go check each one individually and see if it
has a conflict or not... quite annoying!
...
The GraphQL query I ended up writing to fetch data about PRs looks like this:
...
The next step is turning this JSON string we get as a response -- which is quite
hard to work with -- into a list of `PullRequest`s; one can do that in Gleam by
using a [`Decoder`](https://hexdocs.pm/gleam_stdlib/gleam/dynamic/decode.html).

The nice thing about them is that they can compose nicely so we can start
...
increasingly more powerful decoder: we start with somthing that can decode just
...
all I know about refactoring and organising code applies just as nicely to my
HTML-producing code.
...
Ok we all know the joke of the developer speding months trying to automate a
5 minutes task. But this actually took me less than an hour to build -- Gleam is
