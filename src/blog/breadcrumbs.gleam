...
pub fn new(items: List(Item)) -> Element(a) {
  ...
  html.ol([attribute.class("breadcrumb"), attribute.aria_label("Breadcrumb")], {
...
fn item_to_element(item: Item, is_last: Bool) -> Element(a) {
  case item, is_last {
    ...
    Link(name:, ..), True ->
      html.h3([attribute.aria_current("page")], [html.text(name)])
    AnimatedLink(name:, ..), True ->
      html.h3([attribute.aria_current("page"), animate(name)], [html.text(name)])
    BlockLink(name:), True ->
      html.h2(
        [attribute.aria_current("page"), attribute.data("crumb", "block")],
        [
          html.text(name),
        ],
      )

    Link(name:, href:), False ->
      html.a([attribute.href(href)], [html.text(name)])
    AnimatedLink(name:, href:), False ->
      html.a([attribute.href(href), animate(name)], [html.text(name)])
